...
VeloCheck bewertet die Qualität von Veloinfrastruktur und vergibt dafür eine **Schulnote von 1 bis 6** (6 = beste). Grundlage sind zwei Quellen:

- der **[Masterplan Veloinfrastruktur Stadt Bern](https://www.bern.ch/velohauptstadt/infrastruktur/masterplan-veloinfrastruktur)** (welche Führungsform ist wo vorgesehen, welche Querschnitts- und Haltestellen-Vorgaben gelten), und
...
| Ordner | Inhalt |
|---|---|
| [`VeloCheckWeb/`](VeloCheckWeb/) | Die Web-App (React + Vite + TypeScript). |
...
Wo das Velo **auf der Fahrbahn neben möglichem Längsparken** fährt, kann angegeben werden, ob rechts längs geparkt wird (Dooring-Lage). Das betrifft die Fahrbahn-Führungsformen **Mischverkehr, Radstreifen, Velostrasse und Umweltspur** (Konstante `PARKEN_RELEVANT` in [`VeloCheckWeb/src/fuehrungsform.ts`](VeloCheckWeb/src/fuehrungsform.ts)); bei baulich abgesetzten Radwegen und beim Fussweg greift der Dooring-Mechanismus nicht, dort wird das Feld nicht angezeigt. Auswahl **Ja / Nein / Egal**:
...
| GTFS Fahrplan (opentransportdata.swiss) | Bus-Abfahrten 17–18 h je Haltestelle (BPUIC) | Bus-Takt → ÖV-Angebot-Band | gebündelter Snapshot (`VeloCheckWeb/public/oev_takt_bern.json`, via `tools/oev_takt.py`) |
...
| **GTFS Fahrplan** | Bus-Takt (Abendspitze) → ÖV-Angebot-Band | [opentransportdata.swiss](https://opentransportdata.swiss/), Snapshot `VeloCheckWeb/public/oev_takt_bern.json` | Open data — opentransportdata.swiss |
| **OpenBikeSensor** | Gemessene Überholabstände (Info, nicht in der Note) | Portal-Export, Snapshot `VeloCheckWeb/public/obs_bern.json` | „© OpenBikeSensor-Mitwirkende" |
...
| Datei | Zweck |
|---|---|
| [`VeloCheckWeb/src/fuehrungsform.ts`](VeloCheckWeb/src/fuehrungsform.ts) | Bewertungslogik: Soll-Wahl, Führungsform-Note, Breiten-/Parkier-/Haltestellen-Abzug |
| [`VeloCheckWeb/src/App.tsx`](VeloCheckWeb/src/App.tsx) | UI, Eingaben, Import-Wege, Herkunft/Chips, CSV-Export, Entscheidungstabellen |
| [`VeloCheckWeb/src/VeloMap.tsx`](VeloCheckWeb/src/VeloMap.tsx) | Leaflet-Karte: Segmente, Nummern, Haltestellen-Marker, Highlight |
| [`VeloCheckWeb/src/bern.ts`](VeloCheckWeb/src/bern.ts) | Geoportal-Layer (DTV-Formel, Matching, ÖV) |
| [`VeloCheckWeb/src/obs.ts`](VeloCheckWeb/src/obs.ts) | OpenBikeSensor-Überholabstände |
| [`VeloCheckWeb/src/geo.ts`](VeloCheckWeb/src/geo.ts) | Geometrie-Helfer (Überlappungs-Matching) |
| [`tools/oev_takt.py`](tools/oev_takt.py) | GTFS → `VeloCheckWeb/public/oev_takt_bern.json` (Bus-Takt, offline) |
...
Bei jedem Push auf `main`, der `VeloCheckWeb/**` ändert, baut der Workflow
[`.github/workflows/deploy.yml`](.github/workflows/deploy.yml) die App und veröffentlicht sie auf
GitHub Pages. Vite ist dafür auf `base: '/VeloCheck/'` (Build) eingestellt. Einmalig nötig: in den
Repo-Einstellungen unter **Settings → Pages → Source = „GitHub Actions"** aktivieren. Die Seite
erscheint dann unter `https://pnfzygrzgf-svg.github.io/VeloCheck/`.
